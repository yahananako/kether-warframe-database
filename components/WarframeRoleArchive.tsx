...
export default function WarframeRoleArchive(props: ArchiveProps) {
  ...
  const storyMap = useMemo(
    () =>
      new Map(
        warframeStories.map((story) => [
          normalizeWarframeName(story.name),
          story,
        ]),
      ),
    [],
    ...
    () =>
      props.mode === "regular"
        ...
        : props.rows
            .filter(
              (row) =>
                /\bPrime\b/i.test(row.englishName) &&
                isWarframeName(row.englishName),
            )
            .map((row) => ({
  ...
  const classified = useMemo(
    () =>
      entries.map((entry) => {
        ...
        return {
          entry,
          role: getWarframeRole(entry.name, entry.description),
          story:
            normalized === "excalibur" ? undefined : storyMap.get(normalized),
          profile: warframeCanonProfileMap.get(normalized),
        };
      }),
    [entries, storyMap],
  );
  const counts = useMemo(
    () =>
      Object.fromEntries(
        roleOrder.map((item) => [
          item,
          classified.filter((entry) => entry.role === item).length,
        ]),
      ),
    [classified],
  );
  ...
  return (
    <>
      <section className={styles.rolePanel} aria-label="戰甲定位分類">
        <header>
          <span>WARFRAME COMBAT ROLES</span>
          <h2>選擇戰甲定位</h2>
      ...
      <section className={styles.archive}>
        ...
        <div className={styles.frameGrid}>
          {visible.map(({ entry, story, profile }) => {
            ...
            return (
              <article className={styles.frameCard} key={entry.name}>
                ...
                <div className={styles.frameBody}>
                  <p>
                    {entry.chineseName ||
                      (props.mode === "regular" ? "一般戰甲" : "Prime 戰甲")}
                  </p>
                  <h3>{entry.name}</h3>
                  <small>
                  ...
                  {props.mode === "regular" ? (
                    ...
                    <div className={styles.frameMeta}>
                      <b>
                        {entry.price || "價格待更新"}
                        {entry.price && !/白金/.test(entry.price)
                          ? " 白金"
                          : ""}
                      </b>
                      {entry.marketUrl ? (
                  ...
                  {story ? (
                    <details className={styles.lore}>
                      <summary>展開戰甲故事</summary>
                      <strong>{story.epithet}</strong>
                      <p>{story.summary}</p>
                      {story.paragraphs.map((paragraph) => (
                        <p key={paragraph}>{paragraph}</p>
                      ))}
                      <footer>
                        <span>關鍵人物：{story.characters.join("、")}</span>
                        <a
                    ...
                    <details className={styles.lore}>
                      <summary>展開遊戲內故事・{profile.sourceType}</summary>
                      <strong>{profile.epithet}</strong>
                      <p>{profile.summary}</p>
                      {profile.paragraphs.map((paragraph) => (
                        <p key={paragraph}>{paragraph}</p>
                      ))}
                      <footer>
                        <span>資料層級：{profile.sourceType}</span>
                        <a
                  ...
                  <Link
                    ...
                    href={`/database/warframes/${detailSlug}`}
                  >
                    技能、來源與獨立配裝頁 <span>›</span>
                  </Link>
                </div>
              </article>
            );
        ...
        {visible.length === 0 ? (
          <div className={styles.empty}>沒有符合搜尋條件的戰甲。</div>
        ) : null}
      </section>
    </>
  );
