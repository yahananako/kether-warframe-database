...
export default function WarframeRoleArchive({ rows }: { rows: SheetRow[] }) {
  ...
  const classified = useMemo(() => rows.filter((row) => isWarframeName(row.englishName || row.chineseName)).map((row) => ({
    ...
    story: normalizeWarframeName(row.englishName || row.chineseName) === "excalibur" && !/umbra/i.test(row.englishName || row.chineseName)
      ? undefined
      : storyMap.get(normalizeWarframeName(row.englishName || row.chineseName)),
    profile: warframeCanonProfileMap.get(normalizeWarframeName(row.englishName || row.chineseName)),
  })), [rows, storyMap]);
  ...
  return <>
    ...
    <section className={styles.archive}>
      ...
      <div className={styles.frameGrid}>{visible.map(({ row, story, profile }, index) => {
        ...
        return <article className={styles.frameCard} key={`${name}-${index}`}>
          ...
          <div className={styles.frameBody}><p>{row.chineseName || "戰甲"}</p><h3>{name}</h3><small>{row.description || row.note || WARFRAME_ROLES[role].description}</small>
            ...
            {story ? <details className={styles.lore}><summary>展開戰甲故事</summary><strong>{story.epithet}</strong><p>{story.summary}</p>{story.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<footer><span>關鍵人物：{story.characters.join("、")}</span><a href={story.source.url} target="_blank" rel="noreferrer">{story.source.label}</a></footer></details>
              : profile ? <details className={styles.lore}><summary>展開遊戲內故事・{profile.sourceType}</summary><strong>{profile.epithet}</strong><p>{profile.summary}</p>{profile.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<footer><span>資料層級：{profile.sourceType}</span><a href={profile.source.url} target="_blank" rel="noreferrer">{profile.source.label}</a></footer></details>
                : <div className={styles.noLore}>尚無可確認的遊戲內故事資料</div>}
          </div>
        </article>;
      })}</div>
