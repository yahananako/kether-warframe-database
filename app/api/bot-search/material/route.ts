...
import { MATERIAL_ACQUISITION_DATA } from "../../discord/data/materials";

type AnyMaterialRecord = Record<string, unknown>;
...
function pickText(record: AnyMaterialRecord, keys: string[]) {
...
function getSearchValues(record: AnyMaterialRecord) {
  return [
    record.key,
    record.name,
    record.category,
...
function getDetailRows(record: AnyMaterialRecord) {
  return [
    { label: "分類", value: pickText(record, ["category"]) },
    { label: "主要來源", value: pickText(record, ["source"]) },
    { label: "推薦刷法", value: pickText(record, ["recommended"]) },
...
export async function GET(request: NextRequest) {
  ...
  const records = (MATERIAL_ACQUISITION_DATA as AnyMaterialRecord[])
    .map((record) => {
