...
const ALLOWED_CATEGORIES = new Set(["important", "event", "general"]);
const ALLOWED_CREATE_STATUSES = new Set(["draft", "published"]);
...
export async function GET(request: NextRequest) {
  try {
    const managementMode = request.nextUrl.searchParams.get("manage") === "1";
    const access = await getClanAccess(request, {
      requireAdmin: managementMode,
    });

    const announcements = managementMode
      ? await access.sql`
          SELECT
    ...
    return NextResponse.json({
      ...
      matchedAdminRoles: access.matchedAdminRoles.map((role) => ({
        name: role.role_name,
