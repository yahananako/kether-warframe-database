...
export default async function WarframesPage() {
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    {error ? <section className={styles.archive}><h2>戰甲資料讀取失敗</h2><p>{error}</p></section> : <><WarframeRoleArchive rows={rows} /><details className={styles.archive}><summary>開啟完整交易與個人持有資料表</summary><DataTable rows={rows} category="warframes" /></details></>}
  </div></main>;
}
