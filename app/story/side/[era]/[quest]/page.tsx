...
export default async function SideStoryPage({ params }: Props) {
  ...
  const index = sideStoryReadingOrder.findIndex((item) => item.era.slug === era.slug && item.story.slug === story.slug); const previous = sideStoryReadingOrder[index - 1]; const next = sideStoryReadingOrder[index + 1]; const illustration = getSideStoryIllustration(story.slug, story.title);
  return <main className={styles.bookPage} style={{ "--chapter-accent": era.accent } as CSSProperties}><header className={styles.siteHeader}><Link className={styles.brand} href="/"><span className={styles.brandMark}>K</span><span><strong>KETHER</strong><small>SIDE STORY ARCHIVE</small></span></Link><HomeNewInlineMenu /></header><div className={styles.pageShell}>
    ...
    <article className={styles.singlePassage}><header className={styles.passageHeader}><div className={styles.passageNumber}>{era.number}</div><div><p>{era.title}</p><h1>{story.title}</h1><small>{story.englishTitle}</small><strong>{story.period}</strong></div></header><p className={styles.passageLead}>{story.deck}</p><StoryModeSwitcher summary={story.deck} paragraphs={story.paragraphs} characters={story.characters} period={story.period} meaningTitle="本章意義" meaningText={story.meaning} illustration={illustration} /><div className={styles.characterRow}><Users /><span>本章人物</span><div>{story.characters.map((character) => <small key={character}>{character}</small>)}</div></div>
