...
import { CalendarDays, ClipboardList, Database, MessageCircle, Radio, Sparkles } from "lucide-react";
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <header className="rounded-[34px] border border-white/70 bg-white/80 p-5 shadow-2xl shadow-violet-200/30 backdrop-blur sm:p-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-xs font-black tracking-[0.28em] text-violet-600">KETHER OF PARADISO</p>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <article className="rounded-[28px] border border-white/70 bg-white/80 p-5 shadow-xl shadow-violet-200/30 backdrop-blur">
            ...
            <strong className="mt-2 block text-4xl font-black text-slate-950">{totalRows.toLocaleString("zh-TW")}</strong>
            <p className="mt-2 text-sm font-bold text-slate-500">Google Sheets 全分類資料量</p>
          </article>

          <article className="rounded-[28px] border border-white/70 bg-white/80 p-5 shadow-xl shadow-violet-200/30 backdrop-blur">
            <span className="text-sm font-black text-slate-500">有價格資料</span>
            <strong className="mt-2 block text-4xl font-black text-slate-950">{totalPriced.toLocaleString("zh-TW")}</strong>
            <p className="mt-2 text-sm font-bold text-slate-500">目前可追價的資料列</p>
          </article>

          <article className="rounded-[28px] border border-white/70 bg-white/80 p-5 shadow-xl shadow-violet-200/30 backdrop-blur">
            <span className="text-sm font-black text-slate-500">已購買完成率</span>
            <strong className="mt-2 block text-4xl font-black text-slate-950">{totalCompletion}%</strong>
            <p className="mt-2 text-sm font-bold text-slate-500">{totalOwned.toLocaleString("zh-TW")} 筆已標記</p>
          </article>
        </section>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <img src={item.image} alt={item.label} className="h-12 w-12 object-contain transition group-hover:hidden" />
                    <img src={item.activeImage} alt={`${item.label} active`} className="hidden h-12 w-12 object-contain transition group-hover:block" />
                    <div>
                      <strong className="block text-lg font-black text-slate-950">{item.label}</strong>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <article className="relative overflow-hidden rounded-[34px] border border-white/70 bg-white/80 p-6 shadow-2xl shadow-violet-200/30 backdrop-blur">
            <KetherClanWatermark />

            <div className="relative z-10">
              <p className="text-xs font-black tracking-[0.22em] text-violet-600">KETHER STAR CHART INFO</p>
              <h2 className="mt-2 text-3xl font-black text-slate-950">KETHER 星圖航標資訊</h2>
              ...
              <div className="mt-5 grid gap-3">
                <div className="rounded-[22px] bg-violet-50 p-4">
                  <h3 className="font-black text-violet-700">資料庫狀態</h3>
                  <p className="mt-2 text-sm font-bold leading-7 text-slate-600">
                    Google Sheets、分類分頁、區塊總數、交易價格與 Discord 個人進度持續連線中。
                  </p>
                </div>

                <div className="rounded-[22px] bg-slate-50 p-4">
                  <h3 className="font-black text-slate-900">更新備忘錄</h3>
                  <ul className="mt-2 grid gap-2 text-sm font-bold leading-7 text-slate-600">
                    <li>首頁、BOT、總覽與各資料頁上方區塊統一成小希風格。</li>
                    <li>資料頁已加入菜單、放大鏡、鈴鐺與 Discord 快捷入口。</li>
                    <li>/live 小希星圖電波局已上線，支援 Warframe 即時世界狀態。</li>
                    <li>價格更新節奏：每日 4:00。</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <article className="rounded-[34px] border border-white/70 bg-white/80 p-6 shadow-2xl shadow-violet-200/30 backdrop-blur">
            <p className="text-xs font-black tracking-[0.22em] text-violet-600">WARFRAME LINKS</p>
            <h2 className="mt-2 text-3xl font-black text-slate-950">小希星圖網址情報局</h2>
